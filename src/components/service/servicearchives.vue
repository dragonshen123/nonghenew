...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getAddress(e,id, flag,parma,fun){
        var _data = e.$resource(VueResource.data.url+'/webIndexController/'+fun+'?'+parma+'='+id)
        _data.query().then(res=>{
          ...
          if(flag==6){
            e.personal=JSON.parse(res.bodyText)
            // console.log( res.bodyText)
          }
      ...
      getPlantingRecords(e){
        var _data = e.$resource(VueResource.data.url+'/webIndexController/queryPagePlantingrecord')
        _data.query().then(res=>{
          // console.log(res.bodyText)
          e.plant = JSON.parse(res.bodyText)
        })
      }
